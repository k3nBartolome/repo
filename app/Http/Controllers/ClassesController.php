...
use App\Exports\ClassesInformationExport;
...
class ClassesController extends Controller
{
    public function classesInformation(Request $request)
    {
        $query = DB::connection('sqlsrv')
            ...
            ->select(
                DB::raw('ROW_NUMBER() OVER (ORDER BY ApplicantDetails.Id) as No'),
                DB::raw("'TBA' as HRID"),
            ...
            '),
                ...
                DB::raw("
    CASE
        WHEN CHARINDEX('WAVE', Classes.Team) > 0
        THEN 'WAVE ' +
             SUBSTRING(
                 Classes.Team,
                 CHARINDEX('WAVE', Classes.Team) + 5,
                 CASE
                     WHEN PATINDEX('%[^0-9]%', SUBSTRING(Classes.Team, CHARINDEX('WAVE', Classes.Team) + 5, LEN(Classes.Team))) = 0
                     THEN LEN(Classes.Team)
                     ELSE PATINDEX('%[^0-9]%', SUBSTRING(Classes.Team, CHARINDEX('WAVE', Classes.Team) + 5, LEN(Classes.Team))) - 1
                 END
             )
        ELSE ''
    END as Wave
...
DB::raw("'Account Associate' as Position"),
                ...
                'PayRateDetails.NightDifferentialProduction as ND_Production',
                'PayRateDetails.MidShiftProduction as MS_Production',
            ...
            ->whereIn('Status.Id', [76, 82, 83, 84, 85, 86, 88, 89, 90, 92, 93]);
        ...
        try {
            $query = DB::connection('sqlsrv')
                ...
                '),
                    ...
                    "),
                    DB::raw("'Account Associate' as Position"),
                    'SitesDetails.Name as BLDG Assignment',
                    'PayRateDetails.BasicPayTraining as Monthly Salary',
                    'PayRateDetails.NightDifferentialTraining as ND%-Training',
                    'PayRateDetails.NightDifferentialProduction as ND%-Production',
                    'PayRateDetails.MidShiftProduction as Mid-Shift%-Production',
                    'PayRateDetails.ComplexityAllowance as Complexity Allowance'
                )
                ->leftJoin('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.ApplicantApplications as ApplicationDetails', 'ApplicantDetails.Id', '=', 'ApplicationDetails.ApplicantId')
                ->leftJoin('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.Status as Status', 'ApplicationDetails.Status', '=', 'Status.Id')
                ->leftJoin('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.job as JobDetails', 'ApplicationDetails.JobId', '=', 'JobDetails.Id')
    ...
    public function perxStatus()
    {
        $query = DB::connection('sqlsrv')
            ...
            ->select(
                'GeneralStatus as GeneralStatus',
    ...
    public function siteV2(Request $request)
    {
        $query = DB::connection('sqlsrv')
            ->table('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.Sites')
            ->select('Id', 'Name')
    ...
    public function lobV2(Request $request)
    {
        $query = DB::connection('sqlsrv')
            ->table('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.Lob')
            ->select('Name', 'Id')
            ->whereNotNull('Name')
            ->where('isActive', 1);

        if ($request->has('site_id')) {
            $query->where('SiteId', $request->input('site_id'));
        }

        $query->orderBy('Name');

        $lobs = $query->distinct()->get();
    ...
    {
        $minDate = DB::connection('sqlsrv')
            ->table('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.Referrals')->min('DateCreated');
        $maxDate = DB::connection('sqlsrv')
            ->table('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2_PROD.dbo.Referrals')->max('DateCreated');
    ...
    public function perxFilterNoSrExportv2(Request $request)
    {
        ...
        return Excel::download(new ReferralsExport($filteredDataArray), 'perx_data.xlsx');
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'required_program_specific' => 'nullable',
        ...
        if (
            ...
            $class->required_program_specific !== null &&
            ...
            DB::table('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2.dbo.Class')->insert([
                'Team' => $class->team,
                'Site' => $class->site_id, // Mapping site_id to Site
                'Lob' => $class->program_id, // Mapping program_id to Lob
                'ImmediateSupervisorHRID' => $class->immediate_supervisor_hrid,
                'ImmediateSupervisorName' => $class->immediate_supervisor_name,
                'WorkSetup' => $class->work_setup,
                'Status' => 'Ongoing',
                'StartDate' => $class->start_date,
                'EndDate' => $class->end_date,
                'HireDate' => $class->hire_date,
                'TargetHeadcount' => $class->total_target,
                'OfferTarget' => $class->offer_target,
                'OfferCategoryDoc' => $class->offer_category_doc,
                'RequiredProgramSpecific' => $class->required_program_specific,
                'ProgramSpecificId' => $class->program_specific_id,
                'BasicPayTraining' => $class->basic_pay_training,
                'BasicPayProduction' => $class->basic_pay_production,
                'NightDifferentialTraining' => $class->night_differential_training,
                'NightDifferentialProduction' => $class->night_differential_production,
                'BonusTraining' => $class->bonus_training,
                'BonusProduction' => $class->bonus_production,
                'CreatedDate' => $class->created_at,
                'UpdatedDate' => $class->updated_at,
                'IsActive' => true, // Assuming you want to set this to true
            ]);
        }

        // Insert into ClassStaffing table
    ...
    public function pushedback(Request $request, $id)
    {
        ...
        // Create a new instance of the class for the pushed back record
        ...
        // Update the original class record
        $class->fill($request->all());
        $class->changes = 'Add Class';
        $class->status = 'Active';
        ...
        // Calculate Out-of-SLA if applicable
        ...
        // Update or create SmartRecruitClass record
        $smartRecruitClass = DB::table('SMART_RECRUIT.VXI_SMART_RECRUIT_PH_V2.dbo.Class')
            ->updateOrInsert(
                ['Team' => $class->team], // Match the record by Team
                [
                    'Team' => $class->team,
                    'ImmediateSupervisorHRID' => $class->immediate_supervisor_hrid,
                    'ImmediateSupervisorName' => $class->immediate_supervisor_name,

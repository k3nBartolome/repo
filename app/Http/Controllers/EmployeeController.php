...
class EmployeeController extends Controller
{
    ...
    public function ExportEmployee(Request $request, $siteIds = null)
    {
        ...
        if ($request->filled('employee_added_by')) {
            $addedByUsers = explode(',', $request->employee_added_by); // Convert to array
            $employeeQuery->whereIn('employee_added_by', $addedByUsers);
        }
        ...
        $mappedEmployees = $employee_info->map(function ($employee) {
            ...
            return [
                ...
                'bgc_results' => optional($employee->requirements->first())->bgc_results ?? 'N/A',
                ...
                'contract' => optional($employee->workday->first())->contract ?? 'N/A',
                ...
                'with_findings' => optional($employee->workday->first())->with_findings ?? 'N/A',
                'date_endorsed_to_compliance' => optional($employee->workday->first())->date_endorsed_to_compliance ?? 'N/A',
                'return_to_hs_with_findings' => optional($employee->workday->first())->return_to_hs_with_findings ?? 'N/A',
                'last_received_from_hs_with_findings' => optional($employee->workday->first())->last_received_from_hs_with_findings ?? 'N/A',
                'completion' => optional($employee->workday->first())->completion ?? 'N/A',
                'compliance_remarks' => optional($employee->workday->first())->compliance_remarks ?? 'N/A',
    ...
    public function indexModalEmployees(Request $request, $siteIds = null)
    {
        $siteIds = $siteIds ? explode(',', $siteIds) : null;
        $employeeQuery = Employee::with(
            'userAddedBy',
...
'supporting_documents_updated_by' => optional(optional($employee->requirements->first())->supportingDocumentsUpdatedBy)->name ?? 'N/A',
    ...
    public function showUpdate($id)
    {
        ...
        $mappedEmployee = [
            ...
            'contract' => optional($employee->workday->first())->contract,
            'with_findings' => optional($employee->workday->first())->with_findings,
            'date_endorsed_to_compliance' => optional($employee->workday->first())->date_endorsed_to_compliance,
            'return_to_hs_with_findings' => optional($employee->workday->first())->return_to_hs_with_findings,
            'last_received_from_hs_with_findings' => optional($employee->workday->first())->last_received_from_hs_with_findings,
            'compliance_remarks' => optional($employee->workday->first())->compliance_remarks,
            //Program
            'region' => optional($employee->lob->first())->region,
            ...
            'bgc_results' => optional($employee->requirements->first())->bgc_results ?? '',
    ...
    public function updateEmployee(Request $request, $id)
    {
        ...
        try {
            ...
            if ($requirement) {
                ...
                $updateAuditFields('bgc', ['bgc_endorsed_date', 'bgc_results_date', 'bgc_results', 'bgc_final_status', 'bgc_remarks', 'bgc_vendor'], $validatedData, $requirement, $request);
            ...
            if ($workday) {
                $validatedData = $request->validate([
                    ...
                    'compliance_remarks' => 'nullable|string|max:50',
                ]);

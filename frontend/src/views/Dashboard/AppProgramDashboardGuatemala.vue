...
export default {
  ...
  methods: {
    ...
    navigateToEdit(id) {
      this.$router.push(`/program_managementguatemala/edit/${id}`);
    },
    activateProgram(id) {
      const form = {
        is_active: 1,
      ...
      axios
        .put(`http://10.109.2.112:8000/api/programs_activate/${id}`, form, config)
        .then((response) => {
    ...
    deactivateProgram(id) {
      ...
      axios
        .put(
          `http://10.109.2.112:8000/api/programs_deactivate/${id}`,
          form,
          config
        )
    ...
    async getPrograms() {
      try {
        const response = await axios.get(
          "http://10.109.2.112:8000/api/programs7",
          {
            headers: {
              Authorization: `Bearer ${this.$store.state.token}`,
            },
    ...
    async getPrograms2() {
      try {
        const response = await axios.get(
          "http://10.109.2.112:8000/api/programs8",
          {
            headers: {
              Authorization: `Bearer ${this.$store.state.token}`,
            },
    ...
    addProgram() {
      ...
      axios
        .post("http://10.109.2.112:8000/api/programsother", formData, {
          headers: {
            Authorization: `Bearer ${this.$store.state.token}`,
          },
        })

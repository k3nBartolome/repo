<template>
    ...
    <div class="py-8">
      <div class="px-4 py-6 mx-auto bg-white border-2 border-orange-600 max-w-7xl sm:px-6 lg:px-8" >
        <form @submit.prevent="editSite" class="grid grid-cols-1 gap-4 font-semibold sm:grid-cols-2 md:grid-cols-5">
          <label class="block">
            Name
...
export default {
  ...
  methods: {
    async getSites() {
      await axios
        .get("http://i/sites/" + this.$route.params.id)
        .then((response) => {
    ...
    editSite() {
      ...
      axios.put("http://i/sites/"+ this.$route.params.id, formData)
        .then(response => {
          ...
          this.region = '';
          this.updated_by = '';
          this.$router.push("http://i/site_managementguatamela", () => {
            location.reload();
          });
        })
        .catch(error => {

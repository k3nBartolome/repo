...
export default {
  ...
  methods: {
    async getPrograms() {
      try {
        ...
        const response = await axios.get(
          "https://10.236.102.139/api/programs/" + this.$route.params.id,
          {
            headers: {
    ...
    editProgram() {
      ...
      axios
        .put(
          "https://10.236.102.139/api/programsother/" + this.$route.params.id,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        .then((response) => {
          ...
          this.pre_emps = "";
          this.sites_selected = "";
          this.$router.push("/program_managementjamaica", () => {
            location.reload();
          });
        })
        .catch((error) => {

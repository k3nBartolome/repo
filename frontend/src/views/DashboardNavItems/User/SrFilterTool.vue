  ...
  methods: {
    async getSites() {
      try {
        ...
        const response = await axios.get("https://10.236.103.190/api/sr_site", {
          headers: {
    ...
    async getDates() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get("https://10.236.103.190/api/sr_date", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    async fetchData() {
      ...
      try {
        ...
        const response = await axios.get(
          "https://10.236.103.190/api/sr_filter",
          {
            params: {
    ...
    async exportToExcel() {
      ...
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.236.103.190/api/sr_export",
          {
            params: {
              filter_site: this.filterSite,
              filter_date_start: this.filterStartDate,

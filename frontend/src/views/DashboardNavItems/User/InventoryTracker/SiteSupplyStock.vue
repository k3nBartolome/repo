  ...
  methods: {
    ...
    async getInventory(itemId) {
      try {
        ...
        const response = await axios.get(
          `https://10.236.103.168/api/sourcing-item-history/${itemId}`,
          {
            headers: {
    ...
    async getItems() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.236.103.168/api/siteinventoryall",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    transferItems() {
      ...
      axios
        .post("https://10.236.103.168/api/transfer", formData, {
          headers: {
    ...
    addItems() {
      ...
      axios
        .post("https://10.236.103.168/api/items_site_supply", formData, {
          headers: {
            Authorization: `Bearer ${this.$store.state.token}`,
          },
        })

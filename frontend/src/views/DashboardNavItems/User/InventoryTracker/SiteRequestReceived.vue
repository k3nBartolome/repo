      ...
      columns: [
        { data: "id", title: "ID" },
        {
          ...
          render: function (data) {
            return `<button class="w-20 text-xs btn btn-primary" data-id="${data}" onclick="window.vm.receivedRequest(${data})">Received</button>`;
          },
        },
  ...
  methods: {
    ...
    async getInventory() {
      try {
        ...
        const response = await axios.get("http://10.109.2.112:8081/api/inventory/approved/pending", {
          headers: {

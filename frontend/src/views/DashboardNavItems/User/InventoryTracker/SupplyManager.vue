  ...
  methods: {
    ...
    async getInventory(itemId) {
      try {
        ...
        const response = await axios.get(
          `https://10.109.2.112/api/remx-item-history/${itemId}`,
          {
            headers: {
    ...
    async getItems() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.109.2.112/api/itemsboth3",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    transferItems() {
      ...
      axios
        .post("https://10.109.2.112/api/transferremx", formData, {
          headers: {
    ...
    addItems() {
      ...
      axios
        .post("https://10.109.2.112/api/items", formData, {
          headers: {
            Authorization: `Bearer ${this.$store.state.token}`,
          },
        })

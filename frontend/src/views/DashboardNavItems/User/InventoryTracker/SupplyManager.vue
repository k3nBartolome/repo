  ...
  methods: {
    ...
    async getInventory(itemId) {
      try {
        ...
        const response = await axios.get(
          `http://10.109.2.112:8000/api/remx-item-history/${itemId}`,
          {
            headers: {
    ...
    async getItems() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "http://10.109.2.112:8000/api/itemsboth3",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    transferItems() {
      ...
      axios
        .post("http://10.109.2.112:8000/api/transferremx", formData, {
          headers: {
    ...
    addItems() {
      ...
      axios
        .post("http://10.109.2.112:8000/api/items", formData, {
          headers: {
            Authorization: `Bearer ${this.$store.state.token}`,
          },
        })

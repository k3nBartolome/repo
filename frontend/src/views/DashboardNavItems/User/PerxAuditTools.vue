  ...
  methods: {
    async getDates() {
      try {
        ...
        const response = await axios.get(
          "http://10.109.2.112:8000/api/perx_date",
          {
            headers: {
    ...
    async getSites() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "http://10.109.2.112:8000/api/perx_site",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    async fetchData() {
      ...
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "http://10.109.2.112:8000/api/perxfilter",
          {
            params: {
    ...
    async exportToExcel() {
      ...
      try {
        const token = this.$store.state.token;
        const response = await axios.get("http://10.109.2.112:8000/api/export", {
          params: {
            filter_lastname: this.filterLastName,
            filter_firstname: this.filterFirstName,
            filter_site: this.filterSite,

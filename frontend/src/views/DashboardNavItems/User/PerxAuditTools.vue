  ...
  methods: {
    async getDates() {
      try {
        ...
        const response = await axios.get(
          "https://10.236.102.139/api/perx_date",
          {
            headers: {
    ...
    async getSites() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.236.102.139/api/perx_site",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    async fetchData() {
      ...
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.236.102.139/api/perxfilter",
          {
            params: {
    ...
    async exportToExcel() {
      ...
      try {
        const token = this.$store.state.token;
        const response = await axios.get("https://10.236.102.139/api/export", {
          params: {
            filter_lastname: this.filterLastName,
            filter_firstname: this.filterFirstName,
            filter_site: this.filterSite,

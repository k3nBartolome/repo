  ...
  methods: {
    async checkClassExists2() {
      try {
        ...
        const response = await axios.get(
          "https://10.236.102.139/api/class_exists2",
          {
            params: {
    ...
    async getDateRange() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.236.102.139/api/daterange",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    async getPayrate() {
      ...
      try {
        ...
        const response = await axios.get(
          `https://10.236.102.139/api/get_payrate/${this.programs_selected}`,
          { headers }
        );
    ...
    pushClass() {
      ...
      axios
        .put(
          `https://10.236.102.139/api/classes/pushedback/${this.$route.params.id}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,

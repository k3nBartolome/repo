        ...
        <div class="w-full mt-1 md:w-5/5 md:mt-0">
          <label class="block"
            >Remarks<textarea
              ...
              type="text"
              v-model="remarks"
              class="block w-full bg-white border rounded-lg h-15"
            />
          </label>
        </div>
      </div>
  ...
  methods: {
    async checkClassExists2() {
      try {
        ...
        const response = await axios.get(
          "https://10.109.2.112/api/class_exists2",
          {
            params: {
    ...
    async getPayrate() {
      ...
      try {
        ...
        const response = await axios.get(
          `https://10.109.2.112/api/get_payrate/${this.programs_selected}`,
          { headers }
        );
    ...
    async getClasses() {
      try {
        ...
        const response = await axios.get(
          `https://10.109.2.112/api/classes/${this.$route.params.id}`,
          { headers }
        );
        ...
        if (response.status === 200) {
          ...
          this.cl = classObj.cl;
          this.op = classObj.op;
          this.hire_date = classObj.hire_date
            ? new Date(classObj.hire_date).toISOString().split("T")[0]
            : "";
          this.start_date = classObj.start_date
            ? new Date(classObj.start_date).toISOString().split("T")[0]
            : "";
          this.end_date = classObj.end_date
            ? new Date(classObj.end_date).toISOString().split("T")[0]
            : "";
    ...
    async getTransaction() {
      try {
        const token = this.$store.state.token;
        const response = await axios.get(
          "https://10.109.2.112/api/transaction/" + this.$route.params.id,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    pushClass() {
      ...
      axios
        .put(
          `https://10.109.2.112/api/classes/pushedback/${this.$route.params.id}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,

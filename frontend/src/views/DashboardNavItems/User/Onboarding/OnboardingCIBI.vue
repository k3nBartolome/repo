<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">CIBI</h2>
    ...
    <div class="flex flex-col">
      <label class="block text-sm font-medium">CIBI PROOF</label>
      <input
        type="file"
        @change="uploadImage"
        class="p-2 mt-1 border rounded w-full"
    ...
    <div v-if="cibi_file_name" class="mt-4">
...
mounted() {
  ...
  methods: {
    async fetchCibiData() {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get/cibi/requirement/${this.$route.params.id}`
        );
    ...
    async submitForm() {
      ...
      try {
        const apiUrl = `http://127.0.0.1:8000/api/update/cibi/requirement/${this.$route.params.id}`;
    ...
    captureImage() {
      ...
      this.cibi_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.cibi_proof = null;

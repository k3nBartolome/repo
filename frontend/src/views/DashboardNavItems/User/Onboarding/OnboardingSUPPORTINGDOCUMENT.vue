<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">SUPPORTING DOCUMENTS</h2>
    ...
    <div v-if="supporting_documents_file_name" class="mt-4">
...
mounted() {
  ...
  methods: {
    async fetchSdData() {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get/supporting_documents/requirement/${this.$route.params.id}`
        );
        const data = response.data.data;

        // Populate the form fields with API response data
        this.supporting_documents_proof_type = data.supporting_documents_proof_type;
        this.supporting_documents_submitted_date = data.supporting_documents_submitted_date;
        this.supporting_documents_final_status = data.supporting_documents_final_status;
        this.supporting_documents_remarks = data.supporting_documents_remarks;
        this.supporting_documents_file_name = data.supporting_documents_file_name;
      } catch (error) {
    ...
    async submitForm() {
      ...
      if (!this.supporting_documents_final_status) {
        this.supporting_documents_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      if (this.supporting_documents_proof) {
        formData.append(
      ...
      try {
        const apiUrl = `http://127.0.0.1:8000/api/update/supporting_documents/requirement/${this.$route.params.id}`;
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.supporting_documents_proof = file; // Store the file in supporting_documents_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.supporting_documents_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.supporting_documents_proof = compressedFile;
        this.supporting_documents_file_name = dataUrl;
      };
    },
    chooseUpload() {
      this.showUpload = true;
      this.showCapture = false;
      this.supporting_documents_proof = null;
    ...
    captureImage() {
      ...
      this.supporting_documents_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.supporting_documents_proof = null;

<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">BGC</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="flex flex-col">
        <label class="block text-sm font-medium">Date Endorsed</label>
        <input
    ...
    <div v-if="bgc_file_name" class="mt-4">
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      if (!this.bgc_final_status) {
        this.bgc_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      if (this.bgc_proof) {
        formData.append("bgc_proof", this.bgc_proof); // append file here
      }

      try {
        const apiUrl = `http://10.109.2.112:8000/api/update/bgc/requirement/${this.$route.params.id}`;
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.bgc_proof = file; // Store the file in bgc_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.bgc_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.bgc_proof = compressedFile;
        this.bgc_file_name = dataUrl;
      };
    },
    chooseUpload() {
    ...
    captureImage() {
      ...
      this.bgc_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.bgc_proof = null;

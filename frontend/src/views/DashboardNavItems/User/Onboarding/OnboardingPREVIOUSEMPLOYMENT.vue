<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">PREVIOUS EMPLOYMENT</h2>
    ...
    <div class="flex flex-col">
      <label class="block text-sm font-medium">PREVIOUS EMPLOYMENT PROOF</label>
      <input
        type="file"
        @change="uploadImage"
        class="p-2 mt-1 border rounded w-full"
    ...
    <div v-if="previous_employment_file_name" class="mt-4">
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      if (!this.previous_employment_final_status) {
        this.previous_employment_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      if (this.previous_employment_proof) {
        formData.append(
      ...
      try {
        const apiUrl = `http://10.109.2.112:8000/api/update/previous_employment/requirement/${this.$route.params.id}`;
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.previous_employment_proof = file; // Store the file in previous_employment_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.previous_employment_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.previous_employment_proof = compressedFile;
        this.previous_employment_file_name = dataUrl;
      };
    },
    chooseUpload() {
      this.showUpload = true;
      this.showCapture = false;
      this.previous_employment_proof = null;
    ...
    captureImage() {
      ...
      this.previous_employment_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.previous_employment_proof = null;

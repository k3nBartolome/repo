 ...
 mounted() {
  ...
  methods: {
    async fetchBcData() {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get/birth_certificate/requirement/${this.$route.params.id}`
        );
        const data = response.data.data;

        // Populate the form fields with API response data
        this.birth_certificate_proof_type = data.birth_certificate_proof_type;
        this.birth_certificate_submitted_date = data.birth_certificate_submitted_date;
        this.birth_certificate_final_status = data.birth_certificate_final_status;
        this.birth_certificate_remarks = data.birth_certificate_remarks;
        this.birth_certificate_file_name = data.birth_certificate_file_name;
      } catch (error) {
    ...
    async submitForm() {
      ...
      try {
        const apiUrl = `http://127.0.0.1:8000/api/update/birth_certificate/requirement/${this.$route.params.id}`;

<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">HEALTH CERTIFICATE</h2>
    ...
    <div class="flex flex-col">
      <label class="block text-sm font-medium">HEALTH CERTIFICATE PROOF</label>
      <input
        type="file"
        @change="uploadImage"
        class="p-2 mt-1 border rounded w-full"
    ...
    <div v-if="health_certificate_file_name" class="mt-4">
...
mounted() {
  ...
  methods: {
    async fetchHCData() {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get/health_certificate/requirement/${this.$route.params.id}`
        );
    ...
    async submitForm() {
      ...
      // Check if 'health_certificate_final_status' is selected
      if (!this.health_certificate_final_status) {
        this.health_certificate_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      // Append the actual file (health_certificate_proof) for upload
      if (this.health_certificate_proof) {
        formData.append(
      ...
      try {
        const apiUrl = `http://127.0.0.1:8000/api/update/health_certificate/requirement/${this.$route.params.id}`;
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.health_certificate_proof = file; // Store the file in health_certificate_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.health_certificate_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.health_certificate_proof = compressedFile;
        this.health_certificate_file_name = dataUrl;
      };
    },
    chooseUpload() {
      this.showUpload = true;
      this.showCapture = false;
      this.health_certificate_proof = null;
    ...
    captureImage() {
      ...
      this.health_certificate_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.health_certificate_proof = null;

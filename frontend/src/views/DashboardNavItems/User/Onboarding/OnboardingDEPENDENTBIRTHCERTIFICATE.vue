  ...
  <div class="container mx-auto p-4">
    ...
    <div class="flex flex-col">
      <label class="block text-sm font-medium"
        >DEPENDENT's BIRTH CERTIFICATE PROOF</label
      >
      <input
        type="file"
        @change="uploadImage"
    ...
    <div v-if="dependent_birth_certificate_file_name" class="mt-4">
  ...
  methods: {
    async fetchDbcData() {
      try {
        ...
        this.dependent_birth_certificate_proof_type = data.dependent_birth_certificate_proof_type;
        this.dependent_birth_certificate_submitted_date = data.dependent_birth_certificate_submitted_date;
        this.dependent_birth_certificate_final_status = data.dependent_birth_certificate_final_status;
        this.dependent_birth_certificate_remarks = data.dependent_birth_certificate_remarks;
        this.dependent_birth_certificate_file_name = data.dependent_birth_certificate_file_name;
      } catch (error) {
    ...
    async submitForm() {
      ...
      if (!this.dependent_birth_certificate_final_status) {
        this.dependent_birth_certificate_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      if (this.dependent_birth_certificate_proof) {
        formData.append(
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.dependent_birth_certificate_proof = file; // Store the file in dependent_birth_certificate_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.dependent_birth_certificate_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.dependent_birth_certificate_proof = compressedFile;
        this.dependent_birth_certificate_file_name = dataUrl;
      };
    },
    chooseUpload() {
      this.showUpload = true;
      this.showCapture = false;
      this.dependent_birth_certificate_proof = null;
    ...
    captureImage() {
      ...
      this.dependent_birth_certificate_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.dependent_birth_certificate_proof = null;

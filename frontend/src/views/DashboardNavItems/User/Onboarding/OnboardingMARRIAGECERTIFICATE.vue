<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">MARRIAGE CERTIFICATE</h2>
    ...
    <div v-if="marriage_certificate_file_name" class="mt-4">
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      if (!this.marriage_certificate_final_status) {
        this.marriage_certificate_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      if (this.marriage_certificate_proof) {
        formData.append(
      ...
      try {
        const apiUrl = `http://10.109.2.112:8000/api/update/marriage_certificate/requirement/${this.$route.params.id}`;
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.marriage_certificate_proof = file; // Store the file in marriage_certificate_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.marriage_certificate_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.marriage_certificate_proof = compressedFile;
        this.marriage_certificate_file_name = dataUrl;
      };
    },
    chooseUpload() {
      this.showUpload = true;
      this.showCapture = false;
      this.marriage_certificate_proof = null;
    ...
    captureImage() {
      ...
      this.marriage_certificate_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.marriage_certificate_proof = null;

...
mounted() {
  ...
  methods: {
    async fetchSrData() {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/get/scholastic_record/requirement/${this.$route.params.id}`
        );
    ...
    async submitForm() {
      ...
      try {
        const apiUrl = `http://127.0.0.1:8000/api/update/scholastic_record/requirement/${this.$route.params.id}`;

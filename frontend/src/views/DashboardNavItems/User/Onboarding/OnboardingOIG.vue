<template>
  <div class="container mx-auto p-4">
    <h2 class="text-xl font-semibold mb-4 text-center">OIG</h2>
    ...
    <div class="flex flex-col">
      <label class="block text-sm font-medium">OIG PROOF</label>
      <input
        type="file"
        @change="uploadImage"
        class="p-2 mt-1 border rounded w-full"
    ...
    <div v-if="oig_file_name" class="mt-4">
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      if (!this.oig_final_status) {
        this.oig_final_status = "NO STATUS"; // or any default string or null
      }
      ...
      if (this.oig_proof) {
        formData.append("oig_proof", this.oig_proof); // append file here
      }

      try {
        const apiUrl = `http://10.109.2.112:8000/api/update/oig/requirement/${this.$route.params.id}`;
    ...
    uploadImage(event) {
      ...
      if (file) {
        this.oig_proof = file; // Store the file in oig_proof

        const reader = new FileReader();
        reader.onload = () => {
          this.oig_file_name = reader.result; // Preview the image
        };
    ...
    resizeImage(file) {
      ...
      img.onload = () => {
        ...
        this.oig_proof = compressedFile;
        this.oig_file_name = dataUrl;
      };
    },
    chooseUpload() {
    ...
    captureImage() {
      ...
      this.oig_proof = canvas.toDataURL("image/png");
    },
    recaptureImage() {
      this.capturedImage = null;
      this.oig_proof = null;

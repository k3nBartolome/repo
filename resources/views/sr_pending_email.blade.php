...
<body style="background-color: #f3f4f6; font-family: Arial, sans-serif; color: #333;">
    ...
    <div>
        ...
        <p style="margin: 10px 0;">Please find below our SR Pending Movement Report as of {{ date('F j, Y') }}(Date
        ...
        <p style="margin: 10px 0;">The H&S Team has a total pending movement of <strong>{{ $totalPendingHS }}</strong> -
            <strong>{{ $mappedResult['3. INITIAL INTERVIEW']['TotalCount'] ?? 'N/A' }}</strong> pending initial
            interviews,
            <strong>{{ $mappedResult['4. BEHAVIORAL INTERVIEW']['TotalCount'] ?? 'N/A' }}</strong> pending behavioral
            interviews,
            <strong>{{ $mappedResult['5. OPERATIONS VALIDATION']['TotalCount'] ?? 'N/A' }}</strong> OV.
        </p>
    </div>
    ...
    <div style="margin: 10px 0">
        <div style="padding: 1rem 2rem 2rem 0;">
            <div
                style="background-color: white; box-shadow: 0 4px 6px 0 hsla(0, 0%, 0%, 0.07), 0 2px 4px 0 hsla(0, 0%, 0%, 0.06); border-radius: 0.5rem; overflow-x: auto; overflow-y: auto;">
                <table style="min-width: 100%; border-collapse: collapse; border: 2px solid #d1d5db;">
                    <thead>
                        <tr class="border-b-4 border-gray-300 bg-gray-100 text-center">
                            ...
                            <th style="padding: 5px; text-align: left; background-color: gray; color: white;">
                                Bridgetowne</th>
                            ...
                            <th style="padding: 5px; text-align: left; background-color: gray; color: white;">Makati
                            </th>
                            <th style="padding: 5px; text-align: left; background-color: gray; color: white;">MOA</th>
                            <th style="padding: 5px; text-align: left; background-color: gray; color: white;">QC North
                                Edsa</th>
